...
func WriteToExcel(data []RowData, fileName string) error {
	...
	xls.SetCellStr(defalut_sheet, getExcelKey(1, 6), "Cost")
	xls.SetCellStr(defalut_sheet, getExcelKey(1, 7), "Speed")
	...
	for k1, rd := range data {
		...
		xls.SetCellValue(defalut_sheet, getExcelKey(k1+2, 6), rd.Cost)
		xls.SetCellValue(defalut_sheet, getExcelKey(k1+2, 7), rd.Speed)
